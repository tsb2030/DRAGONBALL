...
public class Controller implements Initializable{
	...
	@FXML private Circle pointer;	//공 객체
	@FXML private ImageView backBtn;	//백 버튼(뒤로가기 및 일시정지 기능을 포함)
	...
	public void moving(MouseEvent event) {
		...
		KeyValue kv = new KeyValue(pathTransition.cycleCountProperty(), 5);	//path경로 반복 횟수를 5번으로 지정. pathTransition.setCycleCount(5)와 동일합니다!

		KeyFrame kf = new KeyFrame(new Duration(0), onfinished, kv);	//timeline.play();를 하면 onfinished에 의해 pathTransition도 바로 실행 시작
	...
	public void keyValueCreate() {
		/*
		 * 위 정의해 두었던 color값 배열을 이용하여 시간간격을 임의로 바꿔줌에 따라 새로운 키프레임을 생성하는 메소드
		 * 만들어진 키 프레임들은  timeline에서 사용됩니다.
		 * 결과적으로 어떤 색이 몇 초의 값을 갖느냐에 따라 임의의 시간동안 해당 색깔이 유지됩니다.
		 * 이 부분은 허점이 많기에.... 싹 다 날리거나 수정하거나 원하시는대로 수정하셔도 무방합니다!
		 */
